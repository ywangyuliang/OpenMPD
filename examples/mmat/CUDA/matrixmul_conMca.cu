...
__global__ void matrixMul( float* P, float* M, float* N, int Mh, int Mw, int Nw)
{
    ...
    int nEnd  =  nBegin + (Mw-1)*Nw; /* Nh = Mw */
    float Pvalue = 0.0f;

    for (int m = mBegin, n = nBegin; m <= mEnd; m += mStep, n += nStep) {

        /* Load a tile from M and N into the shared memory arrays */
	if (((BLOCK_SIZE * by+ty) < Mh) && ((m+tx < mEnd) < Mw))
            Ms[ty][tx] = M[m + ty*Mw + tx];
	else Ms[ty][tx] = 0;  
	if (((n+ty*Nw) <= nEnd) && ((BLOCK_SIZE * bx + tx) < Nw))
            Ns[ty][tx] = N[n + ty*Nw + tx];
	else Ns[ty][tx] = 0;
    ...
    if (((by * blockDim.y + ty) < Mh) && ((bx * blockDim.x + tx) < Nw))
        P[Nw * (by * blockDim.y + ty) + bx * blockDim.x + tx] = Pvalue;
}
...
void runTest(int argc, char** argv)
{
    ...
    int block_size = BLOCK_SIZE; /* thread number = block_size^2 */
    int check=0;
    ...
    dim3 block;
    dim3 grid;
    grid.x = (Pw+block_size-1)/block_size;
    grid.y = (Ph+block_size-1)/block_size;
    block.x = block_size;
    block.y = block_size;
    #else
