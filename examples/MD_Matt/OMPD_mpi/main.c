...
// Que pasa con los bucles que llaman a funciones con omp cluster????
//#pragma omp cluster teams broad(movemx, npart,side,rcoff)
MPI_Bcast(&movemx, 1, MPI_INT, 0, MPI_COMM_WORLD);
MPI_Bcast(&npart, 1, MPI_INT, 0, MPI_COMM_WORLD);
MPI_Bcast(&side, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
MPI_Bcast(&rcoff, 1, MPI_DOUBLE , 0, MPI_COMM_WORLD);
...
// Como se llama varias veces a forces sin que TODOS hagan el bucle???
if (__taskid == 0) 
