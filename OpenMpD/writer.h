...
extern long posTaskGlobalDefs;
extern long posTaskGlobalDefsCursor;

void calcularDistribute(string *ini, string *fin, char **linea);
bool comprobarLlavesCluster(char *linea);

void finalize_and_emit_tasking_region(cluster_stack *c){
    ...
    if(posTaskGlobalDefs == -100 || posTaskGlobalDefsCursor == -100){
        fprintf(stderr, "Error: task global_defs slot not reserved\n");
        exit(901);
    }

    rewind(tmp_global_defs);
    posActual = output.tellp();
    output.seekp(posTaskGlobalDefsCursor);

    while((n = fread(buffer, 1, sizeof(buffer), tmp_global_defs)) > 0){
    ...
    rewind(tmp_body);
    while((n = fread(buffer, 1, sizeof(buffer), tmp_body)) > 0){
        output.write(buffer, n);
    }
...
static void close_pending_cluster_if_needed(cluster_stack *c)
{
    ...
    if(c->has_task_pragma){
        ...
        output << "\t/* implicit task synchronization at end of cluster */" << endl;
...
void updateText() {
    ...
    if(ta_expect_task_body || ta_in_task_body_parse){
        if(strcmp(_yytext, "\n") == 0){
            if(linea != NULL){
                free(linea);
                linea = NULL;
    ...
    if(strcmp(_yytext, "\n") != 0) {
        ...
        */
        ...
        /* tasking */
        if(linea != NULL && !enSecuencial && !cluster_stack_isEmpty()){
            cluster_stack* c = cluster_stack_peek();
            if(c != NULL && c->tasking_region != NULL){

                /* normal blocks */
                if(tasking_region_begin_normal_block(c->tasking_region, linea) != 0){
