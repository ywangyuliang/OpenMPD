...
int main(int argc, char *argv[]) 
{
    ...
    // Asignación de memoria para la imagen original y la filtrada
    ...
    // En cada iteración, la salida anterior pasa a ser la nueva entrada.
    for (int iter = 0; iter < iterations; iter++) {
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                // Bordes: se copia el píxel de la entrada de esta iteración
