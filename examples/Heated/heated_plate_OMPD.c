...
#pragma omp cluster broad(epsilon, diff, M,N) alloc(u[M][N]) scatter(w[M][N]) halo(w[M][N]:1*N) gather(w[M][N])
{
#pragma omp teams
  while ( epsilon <= diff )
  {
...
//  The new solution W is the average of north, south, east and west neighbors.
    ...
    #pragma omp parallel for private(aux_diff,j) reduction(max:diff)
    for ( i = 1; i < M - 1; i++ )
    {
      for ( j = 1; j < N - 1; j++ )
      {
    ...
    iterations++;
#pragma omp cluster update halo(w[M][N]:1*N))
