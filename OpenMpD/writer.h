...
extern task_async_context* ta_ctx;
extern int ta_expect_task_body;
extern int ta_num_teams;
extern int ta_rank_counter;

enum {TA_NORMAL, TA_FOR, TA_BODY};
static int ta_state = TA_NORMAL;
static int ta_for_depth = 0;
static int ta_body_depth = 0;
static std::string ta_body;
...
//HALO
extern int pendingHaloDistribute;
extern std::string pendingHaloCode;
extern void clearDeclaredHalos();

extern void MPINumTeams();
...
static void count_braces(const char* s, int &c){
    for(const char* pos = s; pos != NULL && *pos != '\0'; ++pos){
        if(*pos == '{'){
            c++;
        } else if (*pos == '}'){
            c--;
...
void updateText() {
    if(strcmp(_yytext, "\n") != 0) {
        ...
        */
        ...
        if(enMain > 0 && zonaPragma == 1 && finalizeOK == 0 && !cluster_stack_isEmpty() && !enSecuencial) {
        ...
        /* tasjk_async */
        if(linea != NULL && ta_ctx != NULL && !enSecuencial && !cluster_stack_isEmpty()){

            if(ta_state == TA_NORMAL){
                if(strstr(linea, "for") != NULL){
                    task_async_context_begin_for(ta_ctx, linea);
                    ta_for_depth = 0;
                    count_braces(linea, ta_for_depth);
            ...
            if(ta_state == TA_FOR){
                if(ta_expect_task_body && strchr(linea, '{')){
                    ...
                    if(ta_for_depth == 0){
                        ...
                        if(task_async_context_finalize(ta_ctx, tmp, "0", "__taskid", "__status") != 0){
                            fclose(tmp);
                            exit(911);
                        }
                        ...
                        ta_ctx = task_async_context_create(ta_num_teams);
                        ta_rank_counter = 0;
                        ta_state = TA_NORMAL;
                        free(linea);
                        linea = NULL;
            ...
            if(ta_state == TA_BODY){
                ta_body += linea;
                ta_body += "\n";
                count_braces(linea, ta_body_depth);
                free(linea);
                linea = NULL;
                if(ta_body_depth == 0){
                    task_async_context_add_body(ta_ctx, ta_body.c_str());
                    ta_state = TA_FOR;
                    ta_body.clear();
                }
                return;
        ...
        if (enDistribute && enFor == 0) {
            ...
            if(statementZone == 1 && zonaPragma == 0 && finalizeOK == 0) {
        ...
        if (enDistribute && dist_n_llaves == -100) {
            ...
            if (pendingHaloDistribute) {
                output << pendingHaloCode << endl;
                pendingHaloCode = "";
                pendingHaloDistribute = 0;
            }
