...
// HALO
extern std::vector<const char *> args;
extern void addDeclaredHalo(const char *arg);
extern int isHaloDeclared(const char *arg);

extern std::string translateTypes(std::string type);

/* ------------------ task_aync ------------------ */
task_async_context* ta_ctx = NULL;
depend_kind ta_current_kind = DEPEND_IN;
int ta_expect_task_body = 0;
int ta_rank_counter = 0;
int ta_num_teams = 0;
...
cluster_update_directive : CLUSTER UPDATE { cluster_stack_push(CL_NOW); update = 1; } cluster_update_clause_optseq { cluster_stack_close(); }
			 ; 

cluster_teams_directive : CLUSTER TEAMS { cluster_stack_push(CL_ONE_STMT); } cluster_teams_clause_optseq
			;
...
cluster_teams_master_directive : CLUSTER TEAMS MASTER {enMaster = 1; master_n_llaves = 0; master_closes_by_braces = 0; MPIEmpezarSecuencial();} 
							   ;
...
task_async_directive : TASK_ASYNC {
  ...
  int rank = ta_rank_counter % ta_num_teams;
  if(task_async_context_add_task(ta_ctx, rank) != 0){
    exit(902);
  }
  ta_rank_counter++;    
...
dependance_type : IN  { ta_current_kind = DEPEND_IN; }
				        | OUT { ta_current_kind = DEPEND_OUT; }
				        ;
...
var_list_task_async   : variable {
                          ...
                          if(task_async_context_add_depend(ta_ctx, ta_current_kind, chunk, mpiType.c_str()) != 0){
                            exit(904);
                          }
                      } 
                      | var_list_task_async ',' variable {
...
halo_clause : HALO { } '(' var_chunk ')'
    {
        if (update) {
            if (!isHaloDeclared(args.at(0))) {
                fprintf(stderr, "Error: update halo requiere declarar halo(...) previamente en cluster\n");
                exit(130);
            }
