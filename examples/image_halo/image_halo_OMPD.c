...
int main(int argc, char *argv[]) 
{
    ...
    #pragma omp cluster broad(filter_dimension,halo_w,filter_type,iterations)
    ...
    #pragma omp cluster broad(img[height][width]) gather(filtered[height][width]) halo(img[height][width]:halo_w*width)
    {
        // Aplicación del filtro según filter_type
        for (int iter = 0; iter < iterations; iter++) {
            #pragma omp cluster distribute update halo(img[height][width]:halo_w*width)
