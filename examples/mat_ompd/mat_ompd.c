...
/* Scatter works over the flattened matrix, so each chunk must contain complete rows. */
#pragma omp cluster scatter(matA[fA][cA]:chunk(n*cA)) broad(matB[fB*cB]) gather(matC[fA][cB]:chunk(n*cB))
...
/* Neither OpenMP nor MPI handle non-1-D, non-contiguous variables well; probably not worth attempting */
#pragma omp cluster scatter(matA[fA][cA]:chunk(n*cA)) broad(matB[fB][cB]) gather(matC[fA][cB]:chunk(n*cB))
#pragma omp teams distribute dist_schedule(static,n)
#pragma omp parallel for private(result,j,k)
    for (i=0; i<fA; i++) {
        for (j=0; j<cB; j++) {
            result = 0;
            for (k=0; k<cA; k++) {
                result += matA[i*cA+k] * matB[k*cA + j];
            }
            matC[i*cA + j] = result;
        }
    }
...
int main (int argc, char* argv[])
{
	...
	if ((argc != 4) && (argc != 5)) {
           ...
           C2=atoi(argv[4]); /* Number of columns in B */
           if (C1 != F2)
...
#pragma omp cluster
{
    ...
    gettimeofday(&t, NULL);
    #pragma omp cluster
    Mult_ijk(&matA[0][0], &matB[0][0], &matC[0][0], F1, C1, C2);
