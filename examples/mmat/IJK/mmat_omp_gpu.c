...
/**** Distinta version para PGCC que para gcc *****/

#ifdef PGCC
   #pragma omp target teams distribute parallel for collapse(2) map(to:A[0:fA*cA], B[0:cA*cB]) map(from:C[0:fA*cB])
#else
   #pragma omp target teams distribute map(to:A[0:fA*cA], B[0:cA*cB]) map(from:C[0:fA*cB])
#endif
