...
int gather_paren_depth = 0;
int scatter_paren_depth = 0;
int allgather_paren_depth = 0;

extern bool b_within_variable_list;
...
scatter 				  { scatter_paren_depth = 0;yy_push_state(SCATTER_STATE); return SCATTER; }
gather					  { gather_paren_depth = 0; yy_push_state(GATHER_STATE); return GATHER; }
allgather 				  { allgather_paren_depth = 0;yy_push_state(ALLGATHER_STATE); return ALLGATHER; }
...
<SCATTER_STATE>"("                          { scatter_paren_depth++; return '('; }
<SCATTER_STATE>")"                          { scatter_paren_depth--;
                                              if(scatter_paren_depth == 0) yy_pop_state();
                                              return ')'; }
...
<GATHER_STATE>"("                          { gather_paren_depth++; return '('; }
<GATHER_STATE>")"                          { gather_paren_depth--;
                                             if(gather_paren_depth == 0) yy_pop_state();
                                             return ')'; }
...
<ALLGATHER_STATE>"("                          { allgather_paren_depth++; return '('; }
<ALLGATHER_STATE>")"                          { allgather_paren_depth--;
                                                if(allgather_paren_depth == 0) yy_pop_state();
                                                return ')'; }
...
/* Enters expression scanning from the parser */
extern void openmp_parse_expr() {
        yy_push_state(EXPR_STATE);
}

/* Initializes the OpenMP lexer with a pragma string */
...
/* Starts standalone lexing for one pragma string */
void start_lexer(const char* input) {
    yy_scan_string(input);
}

/* Finishes standalone lexing and clears lexer state */
void end_lexer(void) {
