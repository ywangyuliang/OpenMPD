...
#pragma omp parallel for simd private(j,x,y) schedule(static,1) reduction(max:c_max) num_threads(4)
  for ( i = 0; i < m; i++ )
  {
    y = ( ( float ) (     i     ) * y_max
        + ( float ) ( m - i - 1 ) * y_min )
...
#pragma omp parallel for simd private(j,c) schedule(guided,1) num_threads(4)
  for ( i = 0; i < m; i++ )
  {
    for ( j = 0; j < n; j++ )
    {
