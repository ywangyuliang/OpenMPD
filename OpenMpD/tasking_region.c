...
static const char *normalize_c_type_name(const char *value_type){
    ...
    if(strcmp(value_type, "SHORT") == 0){ 
        return "short"; 
    }
    if(strcmp(value_type, "VOID") == 0){ 
        return "void"; 
    }
    if(strcmp(value_type, "SIGNED") == 0){ 
        return "signed"; 
    }
    if(strcmp(value_type, "UNSIGNED") == 0){ 
        return "unsigned"; 
    }
...
static int parse_depend_key(const char* key, depend_parts_t* out_depend_parts){
    ...
    if(lb == NULL){
        size = strlen(key);
        aux = (char*)realloc(out_depend_parts->name, size + 1);
        if(aux == NULL){
            return -1;
        }

        out_depend_parts->name = aux;
        memcpy(out_depend_parts->name, key, size + 1);
        trim(out_depend_parts->name);
        out_depend_parts->is_array_element = 0;
        return 0;
    }

    if(rb == NULL || cl == NULL || !(lb < cl && cl < rb)){
        return -1;
    }
    ...
    out_depend_parts->name = aux;
    memcpy(out_depend_parts->name, key, size);
    out_depend_parts->name[size] = '\0';
    trim(out_depend_parts->name);

    /* index */
    size = (size_t)(cl - (lb + 1));
    aux = (char*)realloc(out_depend_parts->index, size + 1);
    if(aux == NULL){
        return -1;
    }
    out_depend_parts->index = aux;
    memcpy(out_depend_parts->index, lb + 1, size);
    out_depend_parts->index[size] = '\0';
    trim(out_depend_parts->index);

    /* length */
    size = (size_t)(rb - (cl + 1));
    aux = (char*)realloc(out_depend_parts->length, size + 1);
    if(aux == NULL){
        return -1;
    }
    out_depend_parts->length = aux;
    memcpy(out_depend_parts->length, cl + 1, size);
    out_depend_parts->length[size] = '\0';
...
int task_depend_list_add(
    ...
    if(list->len >= list->cap){
        ...
        new_items = (task_depend_t *)realloc(list->items, (size_t)new_cap * sizeof(task_depend_t));
        if(new_items == NULL){
            return -1;
        }
        memset(new_items + list->cap, 0, (size_t)(new_cap - list->cap) * sizeof(task_depend_t));
        list->items = new_items;
        list->cap = new_cap;
    }

    new_depend = &list->items[list->len];
    new_depend->kind = kind;
    new_depend->obj_kind = obj_kind;
    new_depend->raw_text = sstrdup(raw_text);
    new_depend->name = sstrdup(name);
    new_depend->index = index != NULL ? sstrdup(index) : NULL;
    new_depend->length = length != NULL ? sstrdup(length) : NULL;
    new_depend->value_type = sstrdup(normalize_c_type_name(value_type));
    new_depend->mpi_type = sstrdup(mpi_type);

    if(new_depend->raw_text == NULL || new_depend->name == NULL || new_depend->value_type == NULL || new_depend->mpi_type == NULL){
        free(new_depend->raw_text);
        free(new_depend->name);
        free(new_depend->index);
        free(new_depend->length);
        free(new_depend->value_type);
        free(new_depend->mpi_type);
        new_depend->raw_text = NULL;
        new_depend->name = NULL;
        new_depend->index = NULL;
        new_depend->length = NULL;
        new_depend->value_type = NULL;
        new_depend->mpi_type = NULL;
        return -1;
    }
...
int task_input_data_list_add(task_input_data_list_t *list, const char *field, const char *expr, const char *value_type){
    ...
    task_input_data_t *new_input_data;
    int new_cap;
    ...
    if(list->len >= list->cap){
        new_cap = (list->cap == 0) ? 4 : (list->cap * 2);
        new_items = (task_input_data_t *)realloc(list->items, (size_t)new_cap * sizeof(task_input_data_t));
        if(new_items == NULL){
            return -1;
        }
        memset(new_items + list->cap, 0, (size_t)(new_cap - list->cap) * sizeof(task_input_data_t));
        list->items = new_items;
        list->cap = new_cap;
    }

    new_input_data = &list->items[list->len];
    new_input_data->field = sstrdup(field);
    new_input_data->expr = sstrdup(expr);
    new_input_data->value_type = sstrdup(normalize_c_type_name(value_type));

    if(new_input_data->field == NULL || new_input_data->expr == NULL || new_input_data->value_type == NULL){
        free(new_input_data->field);
        free(new_input_data->expr);
        free(new_input_data->value_type);
        return -1;
    }
...
static int tasking_region_ensure_block_capacity(tasking_region_t *tasking_region){
    ...
    if(tasking_region->blocks_len < tasking_region->blocks_cap){
        return 0;
    }

    new_cap = (tasking_region->blocks_cap == 0) ? 4 : (tasking_region->blocks_cap * 2);
    new_blocks = (tasking_block_t*)realloc(tasking_region->blocks, (size_t)new_cap * sizeof(tasking_block_t));
    if(new_blocks == NULL){
        return -1;
    }

    memset(new_blocks + tasking_region->blocks_cap, 0, (size_t)(new_cap - tasking_region->blocks_cap) * sizeof(tasking_block_t));
    tasking_region->blocks = new_blocks;
    tasking_region->blocks_cap = new_cap;
    return 0;
}
...
tasking_region_t *tasking_region_create(int region_id/*, int owner_depth*/){
    ...
    // tasking_region->owner_cluster_brace_depth = owner_depth;
...
void tasking_region_destroy(tasking_region_t *tasking_region){
    ...
    for(i = 0; i < tasking_region->blocks_len; i++){
...
int tasking_region_get_owner_depth(const tasking_region_t *tasking_region){
    ...
    return tasking_region->owner_cluster_brace_depth;
}
*/
...
int tasking_region_get_num_blocks(const tasking_region_t *tasking_region){
    ...
    return tasking_region->blocks_len;
}
...
int tasking_region_begin_normal_block(tasking_region_t *tasking_region, const char *text){
    ...
    tasking_block = &tasking_region->blocks[tasking_region->blocks_len];
    ...
    tasking_block->block.normal_block->raw_text = sstrdup(text);
...
int tasking_region_begin_task_async_block(tasking_region_t *tasking_region){
    ...
    tasking_region->current_open_block = tasking_region->blocks_len;
    tasking_region->blocks_len++;
...
int tasking_region_begin_taskwait_block(tasking_region_t *tasking_region, const char *raw_text){
    ...
    tasking_block->block.taskwait_block->raw_text = sstrdup(raw_text);
...
int tasking_region_add_task_async_block_depend(tasking_region_t *tasking_region, task_depend_kind_t kind, const char *raw_text, const char *value_type, const char *mpi_type){
    ...
    if(tasking_region->current_open_block < 0 || tasking_region->current_open_block >= tasking_region->blocks_len){
        return -1;
    }
...
static void emit_taskwait_receives_for_visible_outputs(tasking_region_t *tasking_region, int wait_index, FILE *file){
    ...
    for(k = 0; k < wait_index; k++){
        ...
        for(j = 0; j < producer_task->depends.len; j++){
            ...
            for(m = k + 1; m < wait_index && !shadowed; m++){
                ...
                for(n = 0; n < later_task->depends.len; n++){
...
int tasking_region_emit_global_defs(tasking_region_t *tasking_region, FILE *file){
    ...
    for(i = 0; i < tasking_region->blocks_len; i++){
        ...
        if(tasking_block->kind == TB_TASK_ASYNC && tasking_block->block.task_async_block != NULL){
            ...
            if(task_async_block->input_datas.len > 0){
                fprintf(file, "typedef struct {\n");
                for(j = 0; j < task_async_block->input_datas.len; j++){
                    fprintf(file, "    %s %s;\n",
...
int tasking_region_emit_body(tasking_region_t *tasking_region, FILE *file){
    ...
    for(i = 0; i < tasking_region->blocks_len; i++){
        ...
        if(tasking_block->kind == TB_TASK_ASYNC && tasking_block->block.task_async_block != NULL){
            ...
            for(j = 0; j < task_async_block->depends.len; j++){
